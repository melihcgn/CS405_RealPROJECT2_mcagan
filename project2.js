...
class MeshDrawer {
	...
	draw(trans) {
		...
		gl.bindBuffer(gl.ARRAY_BUFFER, this.normalbuffer);
		const normalAttrib = gl.getAttribLocation(this.prog, 'normal');
		...
		updateLightPos(this.lightPosLoc);
	...
	enableLighting(show) {
		...
		if (enableLightingLoc !== null) {
			gl.uniform1i(enableLightingLoc, show ? 1 : 0); // Enable or disable lighting based on the 'show' parameter
		} else {
			console.error("Failed to get the uniform location for enabling lighting.");
		}		/**
	...
	setAmbientLight(ambient) {
		...
		if (ambientLoc !== null) {
			gl.uniform1f(ambientLoc, ambient); // Set the ambient light intensity
		} else {
			console.error("Failed to get the uniform location for ambient light.");
		}		/**
		 * @Task2 : You should implement the lighting and implement this function
		 */
	}
...
const meshFS = `
			...
			void main()
			{
				if(showTex && enableLighting){
					...
					// Normalize the normal vector (assuming it's not normalized)
...
const keys = {};
function updateLightPos(lightP) {
    ...
    // Update the light position in 3D space
    const lightPos = [lightX, lightY, lightZ];

    // Pass the updated light position to the shader
    if (lightP !== null) {
        gl.uniform3fv(lightP, lightPos);
    } else {
        console.error("Failed to get the uniform location for light position.");
    }

    console.log(lightPos); // Check the updated light position vector
}
